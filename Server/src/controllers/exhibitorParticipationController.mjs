    ...
    try {
        ...
        if (
            ...
            return res.status(400).json({
                message:
                    "This event is not available for exhibitor applications"
            });
        }
        ...
        if (
            ...
            return res.status(400).json({
                message:
                    "Exhibitor application deadline has passed"
            });
        }
        ...
        if (existingApplication) {
            if (
                ...
                return res.status(200).json({
                    message:
                        "Exhibitor application submitted again",
                    application:
                        existingApplication
                });
            }
        ...
        if (
            ...
            return res.status(400).json({
                message:
                    "Business name, description and category are required"
            });
        }

        // Create application
        const application =
            await ExhibitorParticipation.create({
                exhibitor: exhibitorId,
                event: eventId,
                businessName,
                businessDescription,
...
const cancelApplication = async (req, res) => {
    try {
        ...
        if (
            application.event &&
            new Date() >= application.event.startDate
        ) {
            return res.status(400).json({
                message:
                    "Application cannot be cancelled after the event has started"
            });
        }
        ...
        res.status(200).json({
            message:
                "Exhibitor application cancelled successfully",
            application
        });

    } catch (error) {
        console.error(
            "Cancel exhibitor application error:",
            error.message
        );
...
const approveApplication = async (req, res) => {
    try {
        ...
        if (application.status === "cancelled") {
            return res.status(400).json({
                message:
                    "Cancelled application cannot be approved"
            });
        }
        ...
        res.status(200).json({
            message:
                "Exhibitor application approved successfully",
            application
        });
...
const rejectApplication = async (req, res) => {
    try {
        ...
        if (application.status === "cancelled") {
            return res.status(400).json({
                message:
                    "Cancelled application cannot be rejected"
            });
        }
        ...
        res.status(200).json({
            message:
                "Exhibitor application rejected successfully",
            application
        });
