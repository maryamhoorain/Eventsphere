  ...
  try {
    ...
    if (existingRegistration) {
      if (existingRegistration.status === "cancelled") {
        ...
        const ticketCode = `EVT-${Date.now()}-${Math.random()
          .toString(36)
          .substring(2, 8)
          .toUpperCase()}`;
...
const getRegistrationById = async (req, res) => {
  try {
    ...
    console.error("Get registration details error:", error.message);
...
const cancelRegistration = async (req, res) => {
  try {
    ...
    if (registration.status === "attended") {
      return res.status(400).json({
        message: "An attended registration cannot be cancelled",
      });
    }
    ...
    if (new Date() >= registration.event.startDate) {
      return res.status(400).json({
        message: "Registration cannot be cancelled after the event has started",
      });
    }
...
const checkInAttendee = async (req, res) => {
  try {
    ...
    if (registration.status === "cancelled") {
      return res.status(400).json({
        message: "Cancelled registration cannot be checked in",
      });
    }
