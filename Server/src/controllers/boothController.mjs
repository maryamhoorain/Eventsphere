...
const createBooth = async (req, res) => {
    try {
        ...
        if (existingBooth) {
            return res.status(400).json({
                message:
                    "This booth number already exists for this event"
            });
        }
...
const assignBooth = async (req, res) => {
    try {
        ...
        // Verify approved exhibitor application
        const application =
        ...
        // Keep participation record synchronized
        ...
        await createNotification({
            ...
            message:
                `Booth ${booth.boothNumber} has been assigned to you for your event participation.`,
...
const deleteBooth = async (req, res) => {
    try {
        ...
        if (booth.status !== "available") {
            return res.status(400).json({
                message:
                    "Only available booths can be deleted"
            });
        }
